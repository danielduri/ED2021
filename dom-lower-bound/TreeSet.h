...
public:
    ...
    std::pair<bool,T> lower_bound(T const& e) const{
        ...
        if(p != NULL){
            encontrado = p->_elem >= e;
            elemento=e;
        }else if(!ascendientes.empty() && ascendientes.top()->_elem > e){
            encontrado=true;
            elemento=ascendientes.top()->_elem;
        }

        std::pair<bool, T> ret = std::pair<bool, T>(encontrado, elemento);
        return ret;
    }
