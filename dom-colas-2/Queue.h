...
public:
    ...
    void colotas(T const & pringado, Queue<T> & colar) {
        Nodo * actual;
        actual = _prim;

        while (actual->_sig!=NULL && actual->_elem != pringado) {
            //continuar buscando mientras no se este en el ultimo elemento Y no se haya encontrado
            //Coste: lineal en n, donde n=numero de elementos de la cola al principio (caso peor)
            actual = actual->_sig;
        }

        if (actual->_sig != NULL || actual->_elem==pringado){
            //procesar si actual NO es el ultimo O es el ultimo Y es el pringado
            //Coste: constante
            this->_numElems += colar._numElems;

            colar._ult->_sig = actual->_sig;
            actual->_sig = colar._prim;

            colar._numElems=0;
            colar._prim = colar._ult = NULL;
        }
