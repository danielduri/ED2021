...
public:
    ...
    int count_interval(const T &lower, const T &upper) const{
	    int count = 0;
        count_interval(lower, upper, _ra, count);
        return count;
    }
...
private:
	...
	*/
	...
	static void count_interval(T const & lower, T const & upper, Nodo *p, int & count){
	    if(p!=NULL){
            if (p->_elem<upper && p->_elem>lower){
                count_interval(lower, upper, p->_dr, count);
                count_interval(lower, upper, p->_iz, count);
                count++;
            }else if(p->_elem<=lower){
                if(p->_elem==lower){
                    count++;
                }
                count_interval(lower, upper, p->_dr, count);
	        }else if(p->_elem>=upper){
                if(p->_elem==upper){
                    count++;
                }
                count_interval(lower, upper, p->_iz, count);
