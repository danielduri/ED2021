...
public:
	...
	*/
	...
	const int numHojasP(){
	    return numHojasP(_ra);
	}

	const int alturaP(){
	    return alturaP(_ra, 0);
	}
...
protected:
    ...
    int const numNodos(Nodo * raiz){
        if(raiz==NULL){
            return 0;
        }else{
            return 1+numNodos(raiz->_iz)+numNodos(raiz->_dr);
        }
    }

    int const numHojasP(Nodo * raiz){
	    if(raiz==NULL){
            return 0;
	    }
	    else if(raiz->_iz==NULL && raiz->_dr==NULL){
            return 1;
	    }else{
            return numHojasP(raiz->_iz) + numHojasP(raiz->_dr);
	    }
	}

    int const alturaP(Nodo * raiz, int ct){
        if(raiz==NULL){
            return ct;
        }
        else
        if(raiz->_iz==NULL && raiz->_dr==NULL){
            ...
            ct++;
            return max(alturaP(raiz->_iz, ct), alturaP(raiz->_dr, ct));
        }
    }
